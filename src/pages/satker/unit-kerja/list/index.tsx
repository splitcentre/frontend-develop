...
const ListUnitKerja = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const url = `/api/unit-kerja?page=${paginationModel.page}&perPage=${paginationModel.pageSize}`
      try {
        const response = await axios.get(
          `/api/unit-kerja?page=${paginationModel.page}&perPage=${paginationModel.pageSize}`,
          {
            headers: {
  ...
  const handleDelete = async (id: any) => {
    try {
      await axios.delete(`/api/unit-kerja/delete/${id}`, {
        headers: {
          Authorization: auth.token,
  ...
  const defaultColumns = [
    ...
    createColumn('unit_organisasi_code', 'Kode Unit Organisasi', 1, 280),
    createColumn('code', 'Kode Unit Kerja', 1, 240),
    createColumn('name', 'Nama Unit Kerja', 1, 280)
  ]
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      window.history.replaceState(null, '', '/satker/unit-kerja/list')
    }, 5000)
  }, [router.query])
  ...
  return (
    <Grid container spacing={6}>
      <Grid item xs={12}>
        <PageHeader
          title={<Typography variant='h5'>Daftar Unit Kerja</Typography>}
          subtitle={<Typography variant='body2'>Data Unit Kerja sebagai master data RKAKP</Typography>}
        />
      </Grid>
      <Grid item xs={12}>
      ...
      <Grid item xs={12}>
        <Card>
          <TableHeader urlToAddPage='/satker/unit-kerja/add' addPageName='Tambah Unit Kerja' />
          <DataGridComponent
