...
const ListLembaga = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await getMethod(`/api/lembaga`, '', auth.token)
        setPaginationModel({
          isLoading: false,
  ...
  const defaultColumns = [
    ...
    createColumn('code', 'Kode Lembaga', 1, 240),
    createColumn('name', 'Nama Lembaga', 1, 280)
  ]
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      window.history.replaceState(null, '', '/satker/lembaga/list')
    }, 5000)
  }, [router.query])
  ...
  return (
    <Grid container spacing={6}>
      <Grid item xs={12}>
        <PageHeader
          title={<Typography variant='h5'>Daftar Lembaga</Typography>}
          subtitle={<Typography variant='body2'>Data lembaga sebagai master data RKAKP</Typography>}
        />
      </Grid>
      <Grid item xs={12}>
      ...
      <Grid item xs={12}>
        <Card>
          <TableHeader urlToAddPage='/satker/lembaga/add' addPageName='Tambah Lembaga' />
          <DataGridComponent
